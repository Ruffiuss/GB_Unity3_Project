...
namespace RollABall
{
    internal sealed class Controllers : IUpdatable, IFixedUpdatable, IInitialization//, ILateUpdatable, ICleanupable
    {
        ...
        internal Controllers Add(IControllable controller)
        {
            ...
            {
                _updatableController.Add(upgradableController);
            }
            ...
            {
                _fixedUpdatableControllers.Add(fiedUpdatableController);
            }

            return this;
        }

        public void Initialiazation() //method of IInitialize interface
        {
            for(var index = 0; index < _initializeControllers.Count; ++index)
            {
                _initializeControllers[index].Initialiazation();
            }
        }
        ...
        public void UpdateTick(float deltaTime) //method of IUpdatable interface
        {
            for(var index = 0; index < _initializeControllers.Count; ++index)
            {
                _initializeControllers[index].Initialiazation(deltaTime);
            }
        }
